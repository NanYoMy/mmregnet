...
class VxmDense(LoadableModel):
    ...
    def forward(self, source, target, source_lab=None,target_lab=None,registration=False):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
            source_lab: source label tensor,
            target_lab: target label tensor
        ...
        if registration:
            y_source_lab = self.transformer(source_lab,pos_flow)
            y_target_lab = self.transformer(target_lab,neg_flow) if self.bidir else None

        # return non-integrated flow field if training
        if  registration:
            return y_source,y_source_lab, pos_flow
        else:
