...
def cm_scan_to_scan(mv_vol_names,fix_vol_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    ...
    """
    zeros = None
    gen_mv = volgen(mv_vol_names, batch_size=batch_size, **kwargs)
    gen_fix = volgen(fix_vol_names, batch_size=batch_size, **kwargs)
    while True:
        scan1 = next(gen_mv)[0]
        scan2 = next(gen_fix)[0]
