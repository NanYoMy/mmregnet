...
from tfop.losses import separable_filter3d,gauss_kernel1d

def NVISimilarity(I,J,s=1):

    dxI = I[:, 1:, 1:, 1:, :] - I[:, 1:, :-1, 1:, :]
    dyI = I[:, 1:, 1:, 1:, :] - I[:, :-1, 1:, 1:, :]
    dzI = I[:, 1:, 1:, 1:, :] - I[:, 1:, 1:, :-1, :]
    dx_I = separable_filter3d(dxI, gauss_kernel1d(s))
    dy_I = separable_filter3d(dyI, gauss_kernel1d(s))
    dz_I = separable_filter3d(dzI, gauss_kernel1d(s))

    dxJ = J[:, 1:, 1:, 1:, :] - J[:, 1:, :-1, 1:, :]
    dyJ = J[:, 1:, 1:, 1:, :] - J[:, :-1, 1:, 1:, :]
    dzJ = J[:, 1:, 1:, 1:, :] - J[:, 1:, 1:, :-1, :]
    dx_J = separable_filter3d(dxJ, gauss_kernel1d(s))
    dy_J = separable_filter3d(dyJ, gauss_kernel1d(s))
    dz_J = separable_filter3d(dzJ, gauss_kernel1d(s))

    cross = tf.abs(dx_I * dx_J + dy_I * dy_J + dz_I * dz_J)
    norm = tf.sqrt(
        (dx_I * dx_I + dy_I * dy_I + dz_I * dz_I + 0.00001) * (dx_J * dx_J + dy_J * dy_J + dz_J * dz_J + 0.00001))

    angle = cross / norm
    d = tf.reduce_mean(angle)
    return -1.0 * d

def AwareNVISimilarity(I,J,aware,s=1):
    ...
    angle = aware*(tf.ones_like(cross)-(cross / norm))
    d = tf.reduce_mean(angle)
    return  d

def gauss_gradientSimilarity(s=1):
    def loss(I,J):
